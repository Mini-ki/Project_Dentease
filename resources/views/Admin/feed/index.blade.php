    ...
    <div class="createData">
        ...
        <div class="body">
            <form action="{{ route('admin.feed.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <input type="text" name="judul_feed" id="judul_feed" value="{{ old('judul_feed') }}">
                @error('judul_feed')
                    <p style="color: red;">{{ $message }}</p>
                @enderror

                <label for="deskripsi">Deskripsi</label>
                <textarea name="deskripsi" id="deskripsi" rows="3" style="width:100%; padding:10px; border: 1px solid; border-radius: 5px;">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p style="color: red;">{{ $message }}</p>
                @enderror
                <br>
        
                <label for="image_file">Pilih Gambar</label>
            ...
            <table id="table-information">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td scope="row">
                            @if($feed->image)
                                <img src="{{ asset('storage/img/uploads/feed/' . $feed->image) }}" alt="{{ $feed->judul_feed }}" style="width:100px; height:auto;">
                            @else
                                No Image
                            @endif
                        </td>
                        ...
                        <td scope="row">{{ $feed->updated_at ? $feed->updated_at->format('Y-m-d H:i:s') : 'N/A' }}</td>
